<?php if (has_term('', W2MB_TAGS_TAX, $listing->post->ID)): ?>
<div class="w2mb-field w2mb-field-output-block w2mb-field-output-block-<?php echo esc_attr($content_field->type); ?> w2mb-field-output-block-<?php echo esc_attr($content_field->id); ?>">
	...
	<?php //echo get_the_term_list($listing->post->ID, W2MB_TAGS_TAX, '', ', ', ''); ?>
		<?php
		$terms = get_the_terms($listing->post->ID, W2MB_TAGS_TAX);
		foreach ($terms as $term): ?>
			<span class="w2mb-label w2mb-label-primary w2mb-tag-label"><?php echo $term->name; ?>&nbsp;&nbsp;<span class="w2mb-glyphicon w2mb-glyphicon-tag"></span></span>
		<?php endforeach; ?>
	</span>
</div>
<?php endif; ?>