...
class w2mb_content_field_email extends w2mb_content_field {
	...
	public function renderInput() {
		if (!($template = w2mb_isTemplate('content_fields/fields/email_input_'.$this->id.'.tpl.php'))) {
	...
	public function renderOutput($listing, $group = null) {
		if (!($template = w2mb_isTemplate('content_fields/fields/email_output_'.$this->id.'.tpl.php'))) {
	...
	public function validateCsvValues($value, &$errors) {
		...
		if (!$validation->valid_email($value))
			$errors[] = esc_html__("Email field is invalid", "W2MB");
		return $value;
	}
	...
	public function renderOutputForMap($location, $listing) {
		...
		return w2mb_renderTemplate('content_fields/fields/email_output_map.tpl.php', array('content_field' => $this, 'listing' => $listing, 'email' => $email), true);
	}
}
?>