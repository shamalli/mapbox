...
class w2mb_media_manager {
	...
	public function __construct() {
		add_action('wp_ajax_w2mb_upload_image', array($this, 'upload_image'));
		add_action('wp_ajax_nopriv_w2mb_upload_image', array($this, 'upload_image'));

		add_action('wp_ajax_w2mb_upload_media_image', array($this, 'upload_media_image'));
		
		add_action('wp_ajax_w2mb_remove_image', array($this, 'remove_image'));
		add_action('wp_ajax_nopriv_w2mb_remove_image', array($this, 'remove_image'));
		...
		if (get_option('w2mb_prevent_users_see_other_media')) {
	...
	public function fix_media_counts($views) {
		...
		$views['all'] = "<a href='upload.php'$class>" . sprintf(__('All <span class="count">(%s)</span>', 'W2MB'), number_format_i18n($_total_posts)) . '</a>';
		...
		$views['detached'] = '<a href="upload.php?detached=1"' . ($detached ? ' class="current"' : '') . '>' . sprintf(__( 'Unattached <span class="count">(%s)</span>', 'W2MB'), $total_orphans) . '</a>';
		return $views;
	}

	public function mediaMetabox($post = null, $args = array()) {
		w2mb_renderTemplate('media_metabox.tpl.php' , array_merge(array(
				'images' => $this->images,
				'videos' => $this->videos,
				'logo_image' => $this->logo_image,
				'classes' => (!empty($args['args']['target']) ? 'w2mb-media-metabox-'.$args['args']['target'] : '')
			), $this->params
		));
	}
	...
	public function upload_image() {
		...
		if (wp_verify_nonce($_GET['_wpnonce'], 'upload_images') && isset($_FILES['browse_file']) && ($_FILES['browse_file']['size'] > 0) && isset($_GET['post_id']) && ($post_id = $_GET['post_id'])) {
			if ($post = get_post($post_id)) {
				if (current_user_can('edit_post', $post_id) || !$post->post->post_author) {
					$do_upload = apply_filters('w2mb_count_attachments', true, $post_id);
					if ($do_upload) {
						...
						if (in_array($uploaded_file_type, $allowed_file_types)) {
							...
							$result['error_msg'] = esc_html__('Please upload only image files (jpg, gif or png).', 'W2MB');
					} else
						$result['error_msg'] = esc_html__('Number of images exceed limit.', 'W2MB');
				} else
					$result['error_msg'] = esc_html__('You do not have permissions to edit this listing!', 'W2MB');
			} else
				$result['error_msg'] = esc_html__('Wrong listing ID.', 'W2MB');
		} else // no file was passed
			$result['error_msg'] = esc_html__('Choose image to upload first!', 'W2MB');
		...
		if ($this->params['images_number'] || $this->params['videos_number']) {
			...
			if ($this->params['images_number']) {
				...
				if ($this->params['logo_enabled']) {
					$validation->set_rules('attached_image_as_logo', esc_html__('Logo image selection', 'W2MB'));
				}
			}
	...
	public function avoid_post_type_switcher($pages) {
		if (w2mb_getValue($_POST, 'post_type') == W2MB_POST_TYPE) {
			return;
		}
		return $pages;
	}
	...
	public function admin_enqueue_scripts_styles() {
		if (get_option('w2mb_images_lightbox') || is_admin()) {
		...
		if (is_admin() && current_user_can('upload_files')) {
			...
			wp_register_script('w2mb_fileupload', W2MB_RESOURCES_URL . 'js/jquery.fileupload.js', array('jquery'), false, true);
			wp_register_script('w2mb_fileupload_iframe', W2MB_RESOURCES_URL . 'js/jquery.iframe-transport.js', array('jquery'), false, true);
			wp_enqueue_script('w2mb_fileupload');
			wp_enqueue_script('w2mb_fileupload_iframe');
