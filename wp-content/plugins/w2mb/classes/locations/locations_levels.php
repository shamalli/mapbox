...
class w2mb_locations_levels {
	...
	public function getLevelsFromDB() {
		...
		$array = $wpdb->get_results("SELECT * FROM {$wpdb->w2mb_locations_levels}", ARRAY_A);
		foreach ($array AS $row) {
			$level = new w2mb_locations_level;
	...
	public function createLevelFromArray($array) {
		...
		$insert_update_args = array(
				...
				'in_address_line' => w2mb_getValue($array, 'in_address_line'),
				'allow_add_term' => w2mb_getValue($array, 'allow_add_term'),
		);
		
		$insert_update_args = apply_filters('w2mb_locations_level_create_edit_args', $insert_update_args, $array);
	
		if ($wpdb->insert($wpdb->w2mb_locations_levels, $insert_update_args)) {
			$new_level_id = $wpdb->insert_id;

			do_action('w2mb_update_locations_level', $new_level_id, $insert_update_args);
	...
	public function saveLevelFromArray($level_id, $array) {
		...
		if ($wpdb->update($wpdb->w2mb_locations_levels, $insert_update_args,	array('id' => $level_id), null, array('%d')) !== false) {
			do_action('w2mb_update_locations_level', $level_id, $insert_update_args);
	...
	public function deleteLevel($level_id) {
		...
		if ($wpdb->delete($wpdb->w2mb_locations_levels, array('id' => $level_id))) {
			$this->getLevelsFromDB();
...
class w2mb_locations_level {
	...
	public function buildLevelFromArray($array) {
		...
		$this->allow_add_term = w2mb_getValue($array, 'allow_add_term');
		$this->in_address_line =w2mb_getValue($array, 'in_address_line');
	}
}
...
class w2mb_manage_locations_levels_table extends WP_List_Table {

	public function __construct() {
		parent::__construct(array(
				'singular' => esc_html__('locations level', 'W2MB'),
				'plural' => esc_html__('locations levels', 'W2MB'),
				'ajax' => false
		));
	}
	...
	public function get_columns() {
		$columns = array(
				'locations_level_name' => esc_html__('Name', 'W2MB'),
				'in_address_line' => esc_html__('In address line', 'W2MB'),
				'allow_add_term' => esc_html__('Allow add location', 'W2MB'),
		);
	...
	public function column_locations_level_name($item) {
		$actions = array(
				'edit' => sprintf('<a href="?page=%s&action=%s&level_id=%d">' . esc_html__('Edit', 'W2MB') . '</a>', $_GET['page'], 'edit', $item['id']),
				'delete' => sprintf('<a href="?page=%s&action=%s&level_id=%d">' . esc_html__('Delete', 'W2MB') . '</a>', $_GET['page'], 'delete', $item['id']),
		);
	...
	function no_items() {
		esc_html__('No locations levels found', 'W2MB');
	}
}

add_action('init', 'w2mb_locations_levels_names_into_strings');
function w2mb_locations_levels_names_into_strings() {
	global $w2mb_instance, $sitepress;

	if (function_exists('wpml_object_id_filter') && $sitepress) {
		foreach ($w2mb_instance->locations_levels->levels_array AS &$locations_level) {
...
add_filter('w2mb_locations_level_create_edit_args', 'w2mb_filter_locations_level_fields', 10, 2);
function w2mb_filter_locations_level_fields($insert_update_args, $array) {
...
add_action('w2mb_update_locations_level', 'w2mb_update_locations_level', 10, 2);
function w2mb_update_locations_level($level_id, $array) {
	global $sitepress;
	
	if (function_exists('wpml_object_id_filter') && $sitepress) {
		if ($sitepress->get_default_language() == ICL_LANGUAGE_CODE) {
			do_action('wpml_register_single_string', 'Web 2.0 Directory', 'The name of locations level #' . $level_id, w2mb_getValue($array, 'name'));
		}
	}
}
